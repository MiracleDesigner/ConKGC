...
class LFramework(nn.Module):
    ...
    def run_train(self, train_data, dev_data):
        ...
        for epoch_id in range(self.start_epoch, self.num_epochs):
            ...
            stdout_msg = 'Epoch {}: average training loss = {}'.format(epoch_id, avg_batch_loss)
            if entropies:
                stdout_msg += ' entropy = {}'.format(np.mean(entropies))
            print(stdout_msg)
            self.save_checkpoint(checkpoint_id=epoch_id, epoch_id=epoch_id, loss=avg_batch_loss)
            if self.run_analysis:
            ...
            if self.run_analysis or (epoch_id > 0 and epoch_id % self.num_peek_epochs == 0):
                ...
                if metrics > best_dev_metrics:
                    self.save_checkpoint(checkpoint_id=epoch_id, epoch_id=epoch_id, loss=avg_batch_loss, is_best=True)
    ...
    def save_checkpoint(self, checkpoint_id, epoch_id=None, loss=None, is_best=False):
        """
